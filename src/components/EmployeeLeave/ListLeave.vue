<template>
    ...
    <div class="table-container">
        ...
        <div v-if="editing">
            <!-- <EditEmployee :employee="employeeToEdit"
...
export default {
    ...
    computed: {
      ...
      return this.employees.filter((employee_Id) =>
        employee.employee_Id.toLowerCase().includes(this.searchTerm.toLowerCase()));
        },
    },
    methods: {
        generateReport() {
  ...
  `;
    ...
    <td style="text-align: center;">{{ employee.department_id }}</td>
    <td style="text-align: center;">{{ employee.leave_type }}</td>
    <td style="text-align: center;">{{ employee.NoOfDays }}</td>
</tr>
      `;
            });
        ...
        async deleteEmployee(employee) {
            try {
                ...
                const response = await fetch(
                    `http://localhost:5030//api/Leave/DeleteEmployeeLeave/{employeeId}`,
                    DELETE,
                    {
                        method: "DELETE",
                    }
