<template>
  <div class="payTransaction-table">
    ...
    <div class="search-container">
      <input type="text" v-model="searchQuery" placeholder="Search by transactionID or customerID">
    </div>
    <table class="table table-striped tea-table">
      ...
      <tbody v-if="filteredTransactions.length > 0">
        <tr v-for="transaction in filteredTransactions" :key="transaction.transactionID">
          ...
          <td>
            <button @click="deleteTransaction(transaction.TransactionID)" class="btn btn-danger">Delete</button><br><br>
            <button @click="updateTransaction(transaction)" class="btn btn-warning">Update</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  computed: {
    filteredTransactions() {
      ...
      return this.transactions.filter(transaction => transaction.transactionID === query);
    }
  ...
  async retrieveTransactions() {
    try {
      const response = await fetch("http://localhost:5030/api/Payment/GetAllPayments");
      if (!response.ok) {
  ...
  async deleteTransaction(transactionID) {
    ...
    try {
      const response = await fetch(`http://localhost:5030/api/Payment/DeletePayment?transactionID=${transactionID}`, {
        method: "DELETE"
      });
      if (!response.ok) {
  ...
  async saveEditedTransaction() {
    if (!this.editedTransaction || !this.editedTransaction.name || !this.editedTransaction.amount || !this.editedTransaction.date)
 {
    ...
    try {
      const response = await fetch("http://localhost:5030/api/Payment/UpdatePayment", {
        method: "PUT",
        headers: {
      ...
      const transactionIndex = this.transactions.findIndex(t => t.TransactionID === this.editedTransaction.TransactionID);
